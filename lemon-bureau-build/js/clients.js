...
import { stackTools, clientsSection } from "./business-content.js";

gsap.registerPlugin(Observer);

const clients = stackTools.map((tool) => ({
  name: tool.name,
  logo: tool.logo,
  url: tool.url,
}));
...
function buildRow(rowIndex) {
  ...
  for (let c = 0; c < COPIES; c++) {
    clients.forEach((client, i) => {
      ...
      item.className = "clients-marquee-item clients-marquee-item--stack";
      item.style.borderRadius = RADII[(i + rowIndex) % RADII.length];
      const linkStart = client.url
        ? `<a href="${client.url}" target="_blank" rel="noopener noreferrer" class="clients-marquee-link" aria-label="${client.name}">`
        : "";
      const linkEnd = client.url ? "</a>" : "";
      item.innerHTML = `${linkStart}<img src="${client.logo}" alt="${client.name}" draggable="false" />${linkEnd}`;
