...
/**
 * Utility class to generate simple 3D models for development and testing
 */
export default class SimpleModelGenerator {
  /**
   * Generate a simple spaceship model
   */
  static createSpaceship(): THREE.Group {
  ...
  /**
   * Generate a simple Mars rover model
   */
  static createMarsRover(): THREE.Group {
    const group = new THREE.Group();
  ...
  /**
   * Generate a simple space station model
   */
  static createSpaceStation(): THREE.Group {
    const group = new THREE.Group();
  ...
  /**
   * Generate a simple satellite model
   */
  static createSatellite(): THREE.Group {
    const group = new THREE.Group();
  ...
  /**
   * Generate a simple planet model
   */
  static createPlanet(): THREE.Group {
    const group = new THREE.Group();
    ...
    // Craters
    const craterGeometry = new THREE.CircleGeometry(0.2, 16);
    ...
    for (let i = 0; i < 10; i++) {
      ...
      crater.lookAt(0, 0, 0);
      crater.position.multiplyScalar(1.001); // Push slightly above surface
  ...
  /**
   * Generate a simple telescope model
   */
  static createTelescope(): THREE.Group {
    const group = new THREE.Group();
  ...
  /**
   * Export a group to a GLTF/GLB file
   */
  static exportToGLTF(group: THREE.Group, callback: (buffer: ArrayBuffer) => void): void {
   ...
   * Note: This function should be called in a Node.js environment, not in the browser
   */
  static async generateAllModels(): Promise<void> {
    // This is a placeholder for server-side model generation
    // In a real implementation, you would save these to files on disk
    console.log('This function is meant to be used in a Node.js environment');
    console.log('For browser usage, use the individual model creation methods');
  }
} 