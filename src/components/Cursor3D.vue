<template>
  <div id="three-cursor" ref="cursorContainer">
    <!-- Fallback cursor element - now visible for debugging -->
...
export default defineComponent({
  ...
  setup() {
    ...
    // Debug mode - set to true to help troubleshoot cursor issues
    ...
    const init = () => {
      ...
      console.log("Initializing Cursor3D component");
      
      // Show the fallback cursor in debug mode
      if (DEBUG_MODE) {
        const fallbackCursor = cursorContainer.value.querySelector('.fallback-cursor') as HTMLElement;
        if (fallbackCursor) {
          fallbackCursor.style.display = 'block';
          console.log("Debug mode enabled: fallback cursor is visible");
        }
      }
    ...
    const onDocumentMouseMove = (event: MouseEvent) => {
      ...
      // Update the fallback cursor position for debugging
      if (cursorContainer.value) {
        const fallbackCursor = cursorContainer.value.querySelector('.fallback-cursor') as HTMLElement;
        if (fallbackCursor) {
          fallbackCursor.style.left = `${event.clientX}px`;
          fallbackCursor.style.top = `${event.clientY}px`;
        }
      }
      ...
      // Check for hover state based on elements under the cursor
      ...
      if (isOverInteractive !== isHovering) {
        ...
        console.log(`Hover state changed to: ${isHovering}`);
      }
    };
    ...
    const setupGlobalInteractiveTracking = () => {
      // Comment out the global style that forces cursor:none everywhere
      ...
      //   [tabindex], [onclick], .clickable, nav *, header *, footer *,
      //   .portfolio *, .services *, #app *, .card, .hero *, .about *,
      //   .process *, .contact *, .tech-stack *, .testimonials * {
      //     cursor: none !important;
      //   }
      // `;
    ...
    const attachInteractiveListeners = () => {
      // Comprehensive selector for interactive elements - expanded to include more potential elements
      const interactiveElements = document.querySelectorAll(
        ...
        'nav *, header *, .hero *, .about *, .services *, ' +
        '.process *, footer *, .contact *, .tech-stack *, .testimonials *'
      );
      
      console.log(`Attaching hover listeners to ${interactiveElements.length} elements`);
      ...
      interactiveElements.forEach(el => {
        ...
        // Critical fix: Do NOT apply cursor:none to buttons, links and interactive elements
        // This was preventing click events from working properly
        if (el.tagName !== 'BUTTON' && el.tagName !== 'A' && 
            !el.classList.contains('dev-btn') && !el.classList.contains('dev-btn-primary') && 
            !el.classList.contains('dev-btn-outline')) {
          // Don't override cursor styles
        } else {
          // For buttons and links, use a special cursor style that works with clicks
          console.log("Respecting existing cursor on:", el);
        }
      });
    };
    
    onMounted(() => {
      // Delay init slightly to ensure DOM is fully ready
      setTimeout(() => {
        init();
        console.log("Cursor3D init called after slight delay");
      }, 100);
...
<style>
/* Keep the cursor visible by removing :root cursor override */
:root {
  /* Let elements keep their natural cursor */
}

/* Allow buttons and links to have a pointer cursor */
a, button, .dev-btn, .dev-btn-primary, .dev-btn-outline {
...
#three-cursor {
  ...
  pointer-events: none !important; /* This is critical - ensures cursor doesn't block clicks */
...
.cursor-canvas {
  ...
  pointer-events: none !important; /* This is critical - ensures canvas doesn't block clicks */
}

/* Fallback cursor for debugging - display controlled by JS */
.fallback-cursor {
  position: fixed;
  width: 20px;
  height: 20px;
